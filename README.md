...
The same top level, drawn by the tools rather than by hand:
[`docs/img/schematic_top.svg`](docs/img/schematic_top.svg) is Yosys' elaborated netlist laid
out by netlistsvg, one box per submodule instance with its real port names, regenerated by
`make images`. It is denser than the diagram above and it is not the readable one, but it
comes from the files the simulator and the synthesiser read, so it is the honest check on
whether the hand drawing left anything out.
...
| `scripts/make_schematic.py` | Yosys netlist to a generated top level schematic |
